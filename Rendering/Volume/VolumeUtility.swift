...
class VolumeUtility {
    ...
    /// NUOVO METODO: Applica un filtro di soglia al volume per eliminare valori deboli
    ...
    /// NUOVO METODO: Calcola gradienti più precisi con differenze centrali più piccole
    ...
    /// NUOVO METODO: Ottiene la normale precalcolata per la posizione specificata
    ...
    // ALTERNATIVA: Ritorna un nuovo volume copiando i dati
    static func createNewVolumeFromCopy(_ volume: Volume) -> Volume? {
        ...
        // Il costruttore crea già un volume con data vuota della giusta dimensione
        // Ora dobbiamo semplicemente copiare manualmente i dati
        
        // Questo approccio è complicato dalla natura immutabile di Data in Volume
        // Dobbiamo utilizzare un approccio più creativo
...
extension Volume {
    ...
    /// Stampa statistiche del volume
    func printStatistics() {
        print("\n--- STATISTICHE VOLUME ---")
        print("Dimensioni: \(dimensions.x) x \(dimensions.y) x \(dimensions.z)")
        print("Spacing: \(spacing.x) x \(spacing.y) x \(spacing.z) mm")
        print("Tipo: \(type == .ct ? "TC" : "RM")")
        print("Bits per voxel: \(bitsPerVoxel)")
        
        if let rescaleSlope = rescaleSlope, let rescaleIntercept = rescaleIntercept {
            print("Parametri di trasformazione: Slope=\(rescaleSlope), Intercept=\(rescaleIntercept)")
        }
        
        // Calcola range di valori
        let (min, max) = calculateMinMaxValues()
        print("Range valori: \(min) - \(max)")
        
        if type == .ct {
            print("Range HU stimato: \(Int(min)) - \(Int(max)) HU")
            
            // Identifica valori tipici in TC
            let aria = -1000 // HU dell'aria
            let acqua = 0    // HU dell'acqua
            let osso = 400   // HU minimo per osso compatto
            
            print("\nSoglie consigliate:")
            print("- Aria/Polmone: \(aria) HU")
            print("- Tessuti molli: \(acqua) HU")
            print("- Osso: \(osso) HU")
        }
        print("--- FINE STATISTICHE ---\n")
    }
    
    /// Crea una versione filtrata sostituendo i valori sotto soglia
    /// Metodo alternativo che non modifica direttamente il campo data
    func createVolumeWithThreshold(minValue: Float) -> Volume? {
        print("Creazione volume filtrato con soglia: \(minValue)")
        
        // Crea un nuovo volume vuoto con le stesse dimensioni
        let bytesPerVoxel = bitsPerVoxel / 8
        let totalSize = dimensions.x * dimensions.y * dimensions.z * bytesPerVoxel
        
        // Crea un buffer per i nuovi dati
        var newData = Data(capacity: totalSize)
        
        // Copia tutti i valori, modificando quelli sotto soglia
        var modificati = 0
            ...
            data.withUnsafeBytes { (buffer: UnsafeRawBufferPointer) in
                if let baseAddress = buffer.baseAddress {
                    ...
                    for z in 0..<dimensions.z {
                        for y in 0..<dimensions.y {
                            for x in 0..<dimensions.x {
                                ...
                                if index < buffer.count / 2 {
                                    ...
                                    let rawValue = uint16Buffer[index]
                                    
                                    // Ottieni il valore in Hounsfield per TC
                                    let signedValue = Int16(bitPattern: rawValue)
                                    let value: Float
                                    
                                    if type == .ct, let slope = rescaleSlope, let intercept = rescaleIntercept {
                                        value = Float(signedValue) * Float(slope) + Float(intercept)
                                    } else {
                                        value = Float(signedValue)
                                    }
                                    
                                    // Applica il filtro di soglia
                                    if value < minValue && value > (minValue - 300) {
                                        // Questo valore è sotto la soglia, imposta a zero
                                        let zeroBytes: [UInt8] = [0, 0]
                                        newData.append(contentsOf: zeroBytes)
                                        modificati += 1
                                    } else {
                                        // Mantieni il valore originale
                                        let valueBytes = withUnsafeBytes(of: rawValue) { Data($0) }
                                        newData.append(valueBytes)
                                    }
                                }
                            }
                        }
                        ...
                        // Log progress
                        if z % 20 == 0 {
                            print("Creazione volume filtrato: \(Int((Float(z) / Float(dimensions.z)) * 100))% completato, \(modificati) voxel modificati")
                        }
                    }
                }
            }
        } else if bytesPerVoxel == 1 {
            // Versione per volumi a 8 bit
            for z in 0..<dimensions.z {
                for y in 0..<dimensions.y {
                    for x in 0..<dimensions.x {
                        let pos = SIMD3<Int>(x, y, z)
                        let value = VolumeUtility.getVoxelValue(self, x, y, z)
                        if value < minValue && value > (minValue - 300) {
                            // Zero value
                            newData.append(0)
                            modificati += 1
                            ...
                            // Mantieni il valore originale
                            if let index = linearIndex(for: pos), index < data.count {
        ...
        // Verifica che il buffer sia delle dimensioni corrette
        if newData.count != totalSize {
            print("⚠️ Errore: i dati creati non hanno le dimensioni attese: \(newData.count) vs \(totalSize)")
            return nil
        }
        
        // Crea un nuovo volume con i dati filtrati
        if let newVolume = VolumeFactory.createVolume(
            data: newData,
            ...
            print("✅ Volume filtrato creato con successo: \(modificati) voxel modificati")
            return newVolume
        } else {
            print("⚠️ Errore nella creazione del volume filtrato")
            return nil

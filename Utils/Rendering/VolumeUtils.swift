...
extension Volume {
    ...
    func applyThresholdFilter(minValue: Float) -> Volume {
        return VolumeUtils.applyThresholdFilter(self, minValue: minValue)
    }
    
    /// Funzione di convenienza per calcolare min/max
    func calculateMinMaxValues() -> (min: Float, max: Float) {
        return VolumeUtils.calculateMinMaxValues(self)
    }
    ...
    func clone() -> Volume {
        ...
        return VolumeUtils.applyThresholdFilter(self, minValue: -10000)
    }
}

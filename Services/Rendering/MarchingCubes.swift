...
class MarchingCubes {
    ...
    func generateMesh(from volume: Volume, isovalue: Float) -> Mesh {
         ...
         */
        let normals = VolumeUtils.calculatePreciseGradients(volume: filteredVolume, downsampleFactor: downsampleFactor)
                ...
                for x in stride(from: startX, to: endX, by: downsampleFactor) {
                    ...
                    if processedCubes % 100 == 0 {
                        ...
                        if currentTime.timeIntervalSince(startTime) > maxProcessingTime {
                            ...
                            let cleanedMesh = MeshUtils.removeDisconnectedComponents(mesh: smoothedMesh, minComponentSize: 100)
                    ...
                    let cubeValues: [Float] = [
                        VolumeUtils.getVoxelValue(filteredVolume, x, y, z),                                                    // 0: Vertice in basso a sinistra davanti
                        VolumeUtils.getVoxelValue(filteredVolume, x+downsampleFactor, y, z),                                   // 1: Vertice in basso a destra davanti
                        VolumeUtils.getVoxelValue(filteredVolume, x+downsampleFactor, y+downsampleFactor, z),                  // 2: Vertice in alto a destra davanti
                        VolumeUtils.getVoxelValue(filteredVolume, x, y+downsampleFactor, z),                                   // 3: Vertice in alto a sinistra davanti
                        VolumeUtils.getVoxelValue(filteredVolume, x, y, z+downsampleFactor),                                   // 4: Vertice in basso a sinistra dietro
                        VolumeUtils.getVoxelValue(filteredVolume, x+downsampleFactor, y, z+downsampleFactor),                  // 5: Vertice in basso a destra dietro
                        VolumeUtils.getVoxelValue(filteredVolume, x+downsampleFactor, y+downsampleFactor, z+downsampleFactor), // 6: Vertice in alto a destra dietro
                        VolumeUtils.getVoxelValue(filteredVolume, x, y+downsampleFactor, z+downsampleFactor)                   // 7: Vertice in alto a sinistra dietro
                    ]
                    ...
                    for i in 0..<12 {
                        if (MarchingCubesTables.edgeTable[cubeIndex] & (1 << i)) != 0 {
                            ...
                            let point = MeshUtils.mix(cubePositions[v1], cubePositions[v2], t: clampedT)
                            ...
                            let normal1 = VolumeUtils.getNormalPrecise(normals, normal1X, normal1Y, normal1Z, width, height, depth, downsampleFactor, filteredVolume)
                            let normal2 = VolumeUtils.getNormalPrecise(normals, normal2X, normal2Y, normal2Z, width, height, depth, downsampleFactor, filteredVolume)
                            ...
                            if length(normal1) > 0.0001 || length(normal2) > 0.0001 {
                                ...
                                intersectionNormals[i] = normalize(MeshUtils.mix(normal1, normal2, t: clampedT))
                            } else {
                    ...
                    if cubeIndex >= 0 && cubeIndex < MarchingCubesTables.triTable.count {
                        ...
                        while i < triangleIndices.count {
                            ...
                            let v1 = MeshUtils.addVertex(intersectionPoints[a], intersectionNormals[a], &vertices, &vertexMap, precision)
                            let v2 = MeshUtils.addVertex(intersectionPoints[b], intersectionNormals[b], &vertices, &vertexMap, precision)
                            let v3 = MeshUtils.addVertex(intersectionPoints[c], intersectionNormals[c], &vertices, &vertexMap, precision)
                            ...
                            if triangleCount > maxTriangles {
                                ...
                                let cleanedMesh = MeshUtils.removeDisconnectedComponents(
    ...
    func smoothMesh(_ mesh: Mesh, iterations: Int = 1, factor: Float = 0.5) -> Mesh {
        return MeshUtils.smoothMesh(mesh, iterations: iterations, factor: factor)
    }
}

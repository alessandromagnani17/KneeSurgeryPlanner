...
class Model3DViewModel: ObservableObject {
    ...
    @Published var markerManager: FiducialMarkerManager?
    ...
    func exportModel() {
        Export3DUtils.exportModel(scene: scene)
    }
    
    /// Esporta il modello 3D con marker
    func exportModelWithMarkers() {
        if let markerManager = markerManager {
            Export3DUtils.exportModelWithMarkers(scene: scene, markerManager: markerManager)
        }
    }
